...
class NostrConnectService {
  ...
  static async waitForConnection(
    ...
    try {
      ...
      const candidate: BunkerSignerInstance = await BunkerSigner.fromURI(
        clientSecretKey,
        uri,
        {
      ...
      this.storeSession({
        publicKey,
        signerPubkey: candidate.bp.pubkey!,
        relays: candidate.bp.relays!,
      })
  ...
  static async restoreSession(): Promise<ConnectionResult> {
    ...
    try {
      ...
      const candidate: BunkerSignerInstance = BunkerSigner.fromBunker(
        clientSecretKey,
        bunkerPointer,
        { pool },
      )
