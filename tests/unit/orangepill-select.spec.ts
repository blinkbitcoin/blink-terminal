...
describe("selectFooter", () => {
  it("returns null when off", async () => {
    expect(await selectFooter("off")).toBeNull()
  })

  it("returns a quote footer in quote mode", async () => {
    const f = await selectFooter("quote", new Date(), { seed: "abc" })
  ...
  it("is deterministic for the same seed", async () => {
    const a = await selectFooter("quote", new Date(), { seed: "deadbeef" })
    const b = await selectFooter("quote", new Date(), { seed: "deadbeef" })
    expect(a).toEqual(b)
  })

  it("differs for different seeds (probabilistically)", async () => {
    const seeds = ["a", "b", "c", "d", "e", "f", "g", "h"]
    const footers = await Promise.all(
      seeds.map((s) => selectFooter("quote", new Date(), { seed: s })),
    )
    const texts = new Set(footers.map((f) => f!.lines[0]))
    // With 280+ quotes and 8 seeds, expect more than one distinct result.
    expect(texts.size).toBeGreaterThan(1)
  })

  it("static mode returns the default URL when none provided", async () => {
    const f = await selectFooter("static")
    expect(f!.kind).toBe("static")
    expect(f!.qr).toBe(DEFAULT_STATIC_URL)
  })

  it("static mode honors a custom URL", async () => {
    const url = "https://example.com/learn"
    const f = await selectFooter("static", new Date(), { staticUrl: url })
    expect(f!.qr).toBe(url)
  })

  it("static mode caption is generic for a custom URL, blog-specific for default", async () => {
    const custom = await selectFooter("static", new Date(), {
    ...
    const def = await selectFooter("static")
    expect(def!.caption).toMatch(/blog/i)
  })

  it("ondate returns a coherent event/link + QR for a curated day", async () => {
    ...
    const f = await selectFooter("ondate", new Date(2026, 0, 3), { seed: "x" })
  ...
  it("ondate heading year is coherent with the body (no year mismatch)", async () => {
    ...
    for (const key of Object.keys(CALENDAR)) {
      ...
      for (const seed of ["a", "b", "c"]) {
        const f = await selectFooter("ondate", new Date(2026, mm - 1, dd), { seed })
  ...
  it("shuffle returns content (link or quote)", async () => {
    const f = await selectFooter("shuffle")
    expect(f).not.toBeNull()
    expect(["event", "quote"]).toContain(f!.kind)
  })

  it("shuffle is varied across many unseeded calls", async () => {
  ...
  it("shuffle is deterministic when seeded", async () => {
    const a = await selectFooter("shuffle", new Date(), { seed: "fixed-seed" })
    const b = await selectFooter("shuffle", new Date(), { seed: "fixed-seed" })
    expect(a).toEqual(b)
  })

  it("ondate falls back to a quote for an empty day", async () => {
    ...
    const f = await selectFooter("ondate", missing, { seed: "y" })
    expect(f!.kind).toBe("quote")
  })

  it("never emits a footer body longer than 420 chars", async () => {
    ...
    for (const mode of ["quote", "shuffle"] as const) {
      for (let i = 0; i < 40; i++) {
        const f = await selectFooter(mode, new Date(), { seed: `s${i}` })
    ...
    for (const key of Object.keys(CALENDAR)) {
      const [mm, dd] = key.split("-").map(Number)
      const f = await selectFooter("ondate", new Date(2026, mm - 1, dd), { seed: "z" })
      for (const line of f!.lines) expect(line.length).toBeLessThanOrEqual(420)
    }
  })

  it("ondate uses the link title (not a giant event) when the event is long", async () => {
    // 10-31 (Bitcoin Whitepaper Day) has a ~15k-char event and a curated link.
    const f = await selectFooter("ondate", new Date(2026, 9, 31), { seed: "k" })
    expect(f!.kind).toBe("event")
    expect(f!.qr).toMatch(/^https?:\/\//)
