/**
 * ThemeCycleOverlay - Selectable theme-cycle overlay (authenticated POS).
 *
 * The Theme menu is a selection list (like Sound Themes): tapping a theme
 * enables it in the logo-tap cycle AND applies it immediately (live preview);
 * tapping an enabled theme removes it from the cycle without switching away.
 * The last enabled theme can't be turned off. The POS logo cycles through only
 * the enabled subset.
 */
...
interface ThemeCycleOverlayProps {
  theme: Theme
  enabledThemes: Theme[]
...
export default function ThemeCycleOverlay({
  theme,
  enabledThemes,
  setEnabledThemes,
  setTheme,
...
}: ThemeCycleOverlayProps) {
  const toggle = (id: Theme): void => {
    const isEnabled = enabledThemes.includes(id)
    if (isEnabled) {
      // Deselect — but never empty the set (keep at least one).
    ...
    // Select — add to the cycle AND apply immediately (live preview).
        ...
        <div className={`${getSubmenuHeaderClasses()} sticky top-0 z-10`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              <button
                onClick={() => setShowThemeCycle(false)}
